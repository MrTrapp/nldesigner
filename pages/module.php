<?php
## Vorlage zum Installieren aus dem AddOn uikit_collection Leader Thomas Skerbis von 'Friends Of REDAXO' 

## Pfad zu den Modulen ....
$dir = '../redaxo/src/addons/nldesigner/example/module';
echo '<p>Pfad: '.$dir.'</p>';
// $input1 = rex_file::get(rex_path::addon($addon, 'example/module/01/input.php'));
$modulesdirs = glob($dir.'/*',GLOB_ONLYDIR);
// siehe unten *** Ordner in Assets kopieren >>> $srcPfad  ***
        ...
        if (rex_request('install') == $module_key) {
            ...
            if ($modul_name == '') {
                ...
                if (array_key_exists('assets_folder',$modul['config'])  && $modul['config']['assets_folder'] != '') {
	                 // *** echo $srcdir; '../redaxo/src/addons/nldesigner/example/module/'; *** //
                    $srcdir = $dir.$module_key.'/'.$modul['config']['assets_folder'];
